...
export async function GET(request: NextRequest) {
  ...
  let openQuery = db.from('cz_records').select('*', { count: 'exact', head: true }).eq('status', 'OPEN');
  let solvedQuery = db.from('cz_records').select('*', { count: 'exact', head: true }).eq('status', 'SOLVED');
